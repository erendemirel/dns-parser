...
use dns_parser::types::{RecordType, EdnsOption};
...
fn main() -> Result<()> {
    ...
    // Add EDNS(0) support with padding to a specific size
    let target_size = 128;
    ...
    let ip = Ipv4Addr::new(93, 184, 216, 34); // example.com's IP
    let ttl = 3600; // 1 hour
    ...
    println!("Response size: {} bytes (with EDNS padding)", response_bytes.len());
    ...
    // Check for EDNS options in response
    if let Some((udp_size, _ext_rcode, version, _flags, options)) = response_message.get_edns_options() {
        println!("EDNS: UDP Buffer Size: {}, Version: {}", udp_size, version);
